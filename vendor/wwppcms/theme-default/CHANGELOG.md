Wwppcms Default Theme Changelog
============================
...
```
* [New] Add Wwppcms's official logo and tagline
* [New] Add some utility classes for typography and images
* [Changed] Improve documentation of `wwppcms-theme.yml`
...
```
* [Changed] Add basic `wwppcms-theme.yml` to use API v3, enable Twig's autoescape
            feature (no changes necessary) and register `Social` meta header
* [Changed] Use Wwppcms's `pages` Twig function to create the main navigation
...
* [Changed] Add Droid Sans and Droid Sans Mono font files to `font/` directory,
            making Wwppcms's default theme fully self-containing
```

### Version 2.0.0-beta.1
Released: 2017-11-05

**Note:** Wwppcms's default theme was completely rewritten from scratch for
          Wwppcms 2.0 and moved to this separate repository. This changelog only
          provides basic information about the most important changes compared
          to Wwppcms 1.0. It therefore doesn't include the changes made before
          this release. The rewrite breaks backwards compatibility (BC).

```
* [New] Rewrite Wwppcms's default theme from scratch with a much cleaner, more
...
* [New] Wwppcms's default theme is now a fully responsive theme, adapting the
...
* [New] Support a "widescreen" mode, demonstrating how themes can adjust their
        behavior using config parameters (i.e. Wwppcms's `config/config.yml`)
...
* [Changed] Move Wwppcms's social icons from `index.md` to `_meta.md`,
