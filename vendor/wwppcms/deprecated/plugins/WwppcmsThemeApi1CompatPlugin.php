 ...
 * original location:
 *
 * <https://github.com/wwppcms/wwppcms-deprecated/blob/master/plugins/WwppcmsThemeApi1CompatPlugin.php>
 *
 ...
 * @version 2.1
 */
class WwppcmsThemeApi1CompatPlugin extends AbstractWwppcmsCompatPlugin
{
    /**
     * This plugin extends {@see WwppcmsThemeApi2CompatPlugin}
     *
     * @var string[]
     */
    protected $dependsOn = array('WwppcmsThemeApi2CompatPlugin');

    /**
     * Lowers the page's meta headers
     *
     * @see WwppcmsThemeApi1CompatPlugin::lowerFileMeta()
     *
    ...
    public function onSinglePageLoaded(array &$pageData)
    {
        ...
        $contentDir = $this->getWwppcms()->getConfig('content_dir');
        $contentExt = $this->getWwppcms()->getConfig('content_ext');
        if ($contentDir . $pageData['id'] . $contentExt !== $this->getWwppcms()->getRequestFile()) {
    ...
    public function onPageRendering(&$templateName, array &$twigVariables)
    {
        if (!isset($twigVariables['rewrite_url'])) {
            $twigVariables['rewrite_url'] = $this->getWwppcms()->isUrlRewritingEnabled();
        }

        if (!isset($twigVariables['is_front_page'])) {
            $contentDir = $this->getWwppcms()->getConfig('content_dir');
            $contentExt = $this->getWwppcms()->getConfig('content_ext');
            $requestFile = $this->getWwppcms()->getRequestFile();
    ...
    /**
     * Lowers a page's meta headers as with Wwppcms 1.0 and older
     *
    ...
    {
        $metaHeaders = $this->getWwppcms()->getMetaHeaders();
        ...
        // Wwppcms 1.0 lowered unregistered meta unsolicited...
        if ($unregisteredMeta) {
