 ...
 * original location:
 *
 * <https://github.com/wwppcms/wwppcms-deprecated/blob/master/plugins/WwppcmsThemeApi2CompatPlugin.php>
 *
 ...
 * @version 2.1
 */
class WwppcmsThemeApi2CompatPlugin extends AbstractWwppcmsCompatPlugin
{
    /**
    ...
    /**
     * Sets WwppcmsThemeApi2CompatPlugin::$twigEscapeStrategy
     *
     * @see WwppcmsThemeApi2CompatPlugin::$twigEscapeStrategy
     *
    ...
    public function onPageRendering(&$templateName, array &$twigVariables)
    {
        ...
        $twigVariables['base_dir'] = rtrim($this->getWwppcms()->getRootDir(), '/');
        $twigVariables['theme_dir'] = $this->getWwppcms()->getThemesDir() . $this->getWwppcms()->getTheme();
    }

    /**
     * Registers WwppcmsPluginApi2CompatPlugin::twigEscapeStrategy() as Twig's
     * default escape strategy
     *
     * @see WwppcmsPluginApi2CompatPlugin::twigEscapeStrategy()
     *
    ...
    public function twigEscapeStrategy($templateName)
    {
        ...
        $twigLoader = $this->getWwppcms()->getTwig()->getLoader();
        if (!$twigLoader instanceof Twig_SourceContextLoaderInterface) {
            throw new RuntimeException(
                "WwppcmsDeprecated compat plugin '" . __CLASS__ . "' requires a 'Twig_SourceContextLoaderInterface' "
        ...
        if ($templatePath) {
            $themePath = realpath($this->getWwppcms()->getThemesDir() . $this->getWwppcms()->getTheme()) . '/';
            if (substr($templatePath, 0, strlen($themePath)) === $themePath) {
                $themeApiVersion = $this->getWwppcms()->getThemeApiVersion();
                return ($themeApiVersion >= WwppcmsDeprecated::API_VERSION_3) ? $escapeStrategy : false;
            }

            $plugin = $this->getPluginFromPath($templatePath);
            if ($plugin) {
                $pluginApiVersion = $this->getWwppcmsDeprecated()->getPluginApiVersion($plugin);
                return ($pluginApiVersion >= WwppcmsDeprecated::API_VERSION_3) ? $escapeStrategy : false;
            }
        }
    ...
    {
        $plugins = $this->getWwppcms()->getPlugins();
        ...
        $rootDir = realpath($this->getWwppcms()->getRootDir()) . '/';
        $vendorDir = realpath($this->getWwppcms()->getVendorDir()) . '/';
        $pluginsDir = realpath($this->getWwppcms()->getPluginsDir()) . '/';
        $themesDir = realpath($this->getWwppcms()->getThemesDir()) . '/';
