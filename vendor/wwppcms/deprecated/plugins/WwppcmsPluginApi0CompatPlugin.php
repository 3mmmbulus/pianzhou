 ...
 * original location:
 *
 * <https://github.com/wwppcms/wwppcms-deprecated/blob/master/plugins/WwppcmsPluginApi0CompatPlugin.php>
 *
 ...
 * @version 2.1
 */
class WwppcmsPluginApi0CompatPlugin extends AbstractWwppcmsPluginApiCompatPlugin
{
    /**
     * This plugin extends {@see WwppcmsPluginApi1CompatPlugin} and
     * {@see WwppcmsThemeApi0CompatPlugin}
     *
     * @var string[]
     */
    protected $dependsOn = array('WwppcmsPluginApi1CompatPlugin', 'WwppcmsThemeApi0CompatPlugin');
     ...
     * @see WwppcmsPluginApi0CompatPlugin::onRequestFile()
     *
     * @var string|null
     */
     ...
     * `CACHE_DIR` were removed wih Wwppcms 1.0, `CONTENT_DIR` existed just in
     * Wwppcms 0.9 and `CONFIG_DIR` existed just for a short time between Wwppcms 0.9
     * and Wwppcms 1.0.
     *
    ...
    {
        if (!defined('ROOT_DIR')) {
            define('ROOT_DIR', $this->getWwppcms()->getRootDir());
        }
        if (!defined('CONFIG_DIR')) {
            define('CONFIG_DIR', $this->getWwppcms()->getConfigDir());
        }
        if (!defined('LIB_DIR')) {
            $wwppcmsReflector = new ReflectionClass('Wwppcms');
            define('LIB_DIR', dirname($wwppcmsReflector->getFileName()) . '/');
        }
        if (!defined('PLUGINS_DIR')) {
            define('PLUGINS_DIR', $this->getWwppcms()->getPluginsDir());
        }
        if (!defined('THEMES_DIR')) {
            define('THEMES_DIR', $this->getWwppcms()->getThemesDir());
        }
        if (!defined('CONTENT_DIR')) {
            define('CONTENT_DIR', $this->getWwppcms()->getConfig('content_dir'));
        }
        if (!defined('CONTENT_EXT')) {
            define('CONTENT_EXT', $this->getWwppcms()->getConfig('content_ext'));
        }
        if (!defined('CACHE_DIR')) {
            $twigConfig = $this->getWwppcms()->getConfig('twig_config');
     ...
     *
     * @see WwppcmsPluginApi0CompatPlugin::$requestFile
     *
    ...
    public function onPagesLoaded(
        ...
        $baseUrl = $this->getWwppcms()->getBaseUrl();
        $baseUrlLength = strlen($baseUrl);
        $urlRewritingEnabled = $this->getWwppcms()->isUrlRewritingEnabled();
    ...
    public function triggerEvent($eventName, array $params = array())
    {
        ...
        $plugins = $this->getWwppcmsDeprecated()->getPlugins(WwppcmsDeprecated::API_VERSION_0);
        $plugins += $this->getWwppcmsDeprecated()->getPlugins(WwppcmsDeprecated::API_VERSION_1);
    ...
    {
        return WwppcmsDeprecated::API_VERSION_0;
    }
}
