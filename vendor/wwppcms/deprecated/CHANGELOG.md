Wwppcms Deprecated Changelog
=========================

**Note:** This changelog only provides technical information about the changes
          introduced with a particular Wwppcms version, and is meant to supplement
...
**Note:** Many versions of `WwppcmsDeprecated` include changes which are not
          explicitly mentioned in this changelog. This primarily concerns
          changes in Wwppcms's plugin API. These changes aren't listed separately
          because they are already listed in Wwppcms's changelog. Only functional
...
* [New] Re-introduce Wwppcms v0.9 config constant `CACHE_DIR`
...
**Note:** Wwppcms's official `WwppcmsDeprecated` plugin was moved to this separate
          repository in preparation for Wwppcms 2.0. Refer to Wwppcms's changelog for
          a list of changes to this plugin before Wwppcms 2.0.

```
* [New] Update plugin to API v2 and add support for all API v1 events
* [New] Keep track of all loaded Wwppcms plugins and distinguish them by the API
        version they use; deprecated events are only triggered on plugins using
        this particular API version (`WwppcmsDeprecated::API_VERSION_*` constants)
* [New] Take care of triggering events on plugins using older API versions;
        ...
        result, old plugin's always depend on `WwppcmsDeprecated` now
...
* [New] Make meta headers on the first level of a page's meta data also
        available using a lowered key (as of Wwppcms 1.0; i.e. `SomeKey: value` is
        now accessible using both `$meta['SomeKey']` and `$meta['somekey']`)
* [New] Add public `WwppcmsDeprecated::triggersApiEvents()` method
* [New] Add public `WwppcmsDeprecated::triggerEvent()` method (and the additional
...
* [Changed] ! Don't pass `$plugins` parameter to API v1 `onPluginsLoaded` event
            by reference anymore; use `Wwppcms::loadPlugin()` instead
* [Changed] ! The API v1 events `onTwigRegistration` and `onMetaHeaders`, as
            ...
            of Wwppcms's event flow and are triggered just once on demand
* [Changed] Improve PHP class docs
* [Changed] A vast number of small improvements and changes...
* [Removed] ! Remove support for `WwppcmsParsePagesContent` plugin
* [Removed] ! Remove support for `WwppcmsExcerpt` plugin
```
