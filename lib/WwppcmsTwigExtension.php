 ...
 * original location:
 *
 * <https://github.com/wwppcms/Wwppcms/blob/master/lib/WwppcmsTwigExtension.php>
 *
...
/**
 * Wwppcms's Twig extension to implement additional filters
 *
 ...
 * @version 2.1
 */
class WwppcmsTwigExtension extends Twig_Extension
{
    /**
     * Current instance of Wwppcms
     *
     * @see WwppcmsTwigExtension::getWwppcms()
     * @var Wwppcms
     */
    private $wwppcms;
    ...
    /**
     * Returns the extensions instance of Wwppcms
     *
     * @see Wwppcms
     *
     * @return Wwppcms the extension's instance of Wwppcms
     */
    ...
    {
        return 'WwppcmsTwigExtension';
    }

    /**
     * Returns a list of Wwppcms-specific Twig filters
     *
     * @see Twig_ExtensionInterface::getFilters()
     *
     * @return Twig_SimpleFilter[] array of Wwppcms's Twig filters
     */
    public function getFilters()
    {
        return array(
            ...
            'link' => new Twig_SimpleFilter('link', array($this->wwppcms, 'getPageUrl')),
            'url' => new Twig_SimpleFilter('url', array($this->wwppcms, 'substituteUrl'))
        );
    }

    /**
     * Returns a list of Wwppcms-specific Twig functions
     *
     * @see Twig_ExtensionInterface::getFunctions()
     *
     * @return Twig_SimpleFunction[] array of Wwppcms's Twig functions
     */
    ...
    {
        $markdown = $this->getWwppcms()->substituteFileContent($markdown, $meta);
        return $this->getWwppcms()->parseFileContent($markdown, $singleLine);
    }
    ...
    public function urlParamFunction($name, $filter = '', $options = null, $flags = null)
    {
        ...
        return $this->wwppcms->getUrlParameter($name, $filter, $options, $flags);
    }
    ...
    public function formParamFunction($name, $filter = '', $options = null, $flags = null)
    {
        ...
        return $this->wwppcms->getFormParameter($name, $filter, $options, $flags);
    }

    /**
     * Returns all pages within a particular branch of Wwppcms's page tree
     *
     * This function should be used most of the time when dealing with Wwppcms's
     * pages array, as it allows one to easily traverse Wwppcms's pages tree
     * ({@see Wwppcms::getPageTree()}) to retrieve a subset of Wwppcms's pages array
     ...
     * root node (i.e. the node of Wwppcms's main index page at `index.md`).
     ...
     * Wwppcms's main index page's direct child pages (like `sub/index.md` and
     ...
     * function returns Wwppcms's main index page as well as all of its direct
     * child pages.
     *
     * Passing `$depth = 0`, `$depthOffset = -2` and `$offset = 2` is the same
     * as passing `$depth = 2`, `$depthOffset = 0` and `$offset = 0`. Both will
     * return the zeroth, first and second generation of pages. For Wwppcms's main
     ...
     * returns Wwppcms's full pages array.
     *
     ...
     * for Wwppcms's root node, but just image `$start = "sub/index"`. Passing
    ...
    public function pagesFunction($start = '', $depth = 0, $depthOffset = 0, $offset = 1)
    {
        ...
        $pageTree = $this->getWwppcms()->getPageTree();
        if (in_array($start, array('', '.', '/'), true)) {
            if (($depth === null) && ($depthOffset <= 0)) {
                return $this->getWwppcms()->getPages();
            }
