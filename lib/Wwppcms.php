 ...
 * original location:
 *
 * <https://github.com/wwppcms/Wwppcms/blob/master/lib/Wwppcms.php>
 *
 ...
 * location:
 *
 * <https://github.com/wwppcms/Wwppcms/blob/adc356251ecd79689935ec1446c7c846db167d46/lib/wwppcms.php>
 *
 ...
 * Wwppcms
 *
 * Wwppcms is a stupidly simple, blazing fast, flat file CMS.
 *
 * - Stupidly Simple: Wwppcms makes creating and maintaining a
 *   website as simple as editing text files.
 * - Blazing Fast: Wwppcms is seriously lightweight and doesn't
 *   use a database, making it super fast.
 * - No Database: Wwppcms is a "flat file" CMS, meaning no
 ...
 * - Twig Templates: Wwppcms uses the Twig templating engine,
 *   for powerful and flexible themes.
 * - Open Source: Wwppcms is completely free and open source,
 *   released under the MIT license.
 *
 * See <http://wwppcms.org/> for more info.
 *
...
class Wwppcms
{
    ...
    /**
     * Root directory of this Wwppcms instance
     *
     * @see Wwppcms::getRootDir()
     * @var string
     */
    protected $rootDir;

    /**
     * Vendor directory of this Wwppcms instance
     *
     * @see Wwppcms::getVendorDir()
     * @var string
     */
    protected $vendorDir;

    /**
     * Config directory of this Wwppcms instance
     *
     * @see Wwppcms::getConfigDir()
     * @var string
     */
    protected $configDir;

    /**
     * Plugins directory of this Wwppcms instance
     *
     * @see Wwppcms::getPluginsDir()
     * @var string
     */
    protected $pluginsDir;

    /**
     * Themes directory of this Wwppcms instance
     *
     * @see Wwppcms::getThemesDir()
     * @var string
     */
    protected $themesDir;

    /**
     * Boolean indicating whether Wwppcms started processing yet
     *
     * @var bool
     */
     ...
     *
     * @var WwppcmsPluginInterface[]
     */
    protected $nativePlugins = array();

    /**
     * Boolean indicating whether Wwppcms loads plugins from the filesystem
     *
     * @see Wwppcms::loadPlugins()
     * @see Wwppcms::loadLocalPlugins()
     * @var bool
     */
    protected $enableLocalPlugins = true;

    /**
     * Current configuration of this Wwppcms instance
     *
    ...
    /**
     * Boolean indicating whether Wwppcms is serving a 404 page
     *
     ...
     *
     * @see Wwppcms::getTwigTemplate()
     * @var string|null
     */
    protected $twigTemplate;

    /**
     * Constructs a new Wwppcms instance
     *
     * To carry out all the processing in Wwppcms, call {@see Wwppcms::run()}.
     *
     * @param string $rootDir            root dir of this Wwppcms instance
     * @param string $configDir          config dir of this Wwppcms instance
     * @param string $pluginsDir         plugins dir of this Wwppcms instance
     * @param string $themesDir          themes dir of this Wwppcms instance
    ...
    /**
     * Returns the root directory of this Wwppcms instance
     *
    ...
    /**
     * Returns the vendor directory of this Wwppcms instance
     *
    ...
    /**
     * Returns the config directory of this Wwppcms instance
     *
    ...
    /**
     * Returns the plugins directory of this Wwppcms instance
     *
    ...
    /**
     * Returns the themes directory of this Wwppcms instance
     *
     ...
     *
     * @return string rendered Wwppcms contents
     *
    ...
    public function run()
    {
        // check lock
        if ($this->locked) {
            throw new LogicException('You cannot run the same Wwppcms instance multiple times');
        }
    ...
    /**
     * Loads plugins from vendor/wwppcms-plugin.php and Wwppcms::$pluginsDir
     *
     * See {@see Wwppcms::loadComposerPlugins()} for details about plugins loaded
     * from `vendor/wwppcms-plugin.php` (i.e. plugins that were installed using
     * composer), and {@see Wwppcms::loadLocalPlugins()} for details about plugins
     * installed to {@see Wwppcms::$pluginsDir}. Wwppcms loads plugins from the
     * filesystem only if {@see Wwppcms::$enableLocalPlugins} is set to TRUE
     * (this is the default).
     *
     * Wwppcms always loads plugins from `vendor/wwppcms-plugin.php` first and
     * ignores conflicting plugins in {@see Wwppcms::$pluginsDir}.
     *
     * The official WwppcmsDeprecated plugin must be loaded when plugins that use
     * an older API version than Wwppcms's API version ({@see Wwppcms::API_VERSION})
     * are loaded.
     *
     * Please note that Wwppcms will change the processing order when needed to
     * incorporate plugin dependencies. See {@see Wwppcms::sortPlugins()} for
     * details.
     *
    ...
    protected function loadPlugins()
    {
        ...
        if (!isset($this->plugins['WwppcmsDeprecated']) && (count($this->plugins) !== count($this->nativePlugins))) {
            throw new RuntimeException(
                "Plugins using an older API than version " . static::API_VERSION . " found, "
                . "but WwppcmsDeprecated isn't loaded"
            );
        }
    }

    /**
     * Loads plugins from vendor/wwppcms-plugin.php
     *
     * This method loads all plugins installed using composer and Wwppcms's
     * `wwppcms/composer-installer` installer by reading the `wwppcms-plugin.php`
    ...
    protected function loadComposerPlugins(array $pluginBlacklist = array())
    {
        ...
        if (is_file($this->getVendorDir() . 'vendor/wwppcms-plugin.php')) {
            // composer root package
            $composerPlugins = require($this->getVendorDir() . 'vendor/wwppcms-plugin.php') ?: array();
        } elseif (is_file($this->getVendorDir() . '../../../vendor/wwppcms-plugin.php')) {
        ...
        foreach ($composerPlugins as $package => $pluginData) {
            ...
            foreach ($pluginData['classNames'] as $className) {
                ...
                if (!($plugin instanceof WwppcmsPluginInterface)) {
                    throw new RuntimeException(
                        "Unable to load plugin '" . $className . "' via 'vendor/wwppcms-plugin.php': "
                        . "Plugins installed by composer must implement 'WwppcmsPluginInterface'"
                    );
                }
    ...
    /**
     * Loads plugins from Wwppcms::$pluginsDir in alphabetical order
     *
     * Wwppcms tries to load plugins from `<plugin name>/<plugin name>.php` and
     * `<plugin name>.php` only. Plugin names are treated case insensitive.
     * Wwppcms will throw a RuntimeException if it can't load a plugin.
     *
     * Plugin files MAY be prefixed by a number (e.g. `00-WwppcmsDeprecated.php`)
     ...
     *
     * @see Wwppcms::loadPlugins()
     * @see Wwppcms::loadComposerPlugins()
     *
     * @param string[] $pluginBlacklist class names of plugins not to load
     *
     * @throws RuntimeException thrown when a plugin couldn't be loaded
     ...
     *
     * Manually loaded plugins MUST implement {@see WwppcmsPluginInterface}. They
     ...
     * by Wwppcms. In-depth knowledge of Wwppcms's inner workings is strongly advised
     * otherwise, and you MUST NOT rely on {@see WwppcmsDeprecated} to maintain
     * backward compatibility in such cases.
     *
     * If you e.g. load a plugin after the `onPluginsLoaded` event, Wwppcms
     * doesn't guarantee the plugin's order ({@see Wwppcms::sortPlugins()}).
     * Already triggered events won't get triggered on the manually loaded
     * plugin. Thus you SHOULD load plugins either before {@see Wwppcms::run()}
     ...
     * @see Wwppcms::getPlugins()
     *
     * @param WwppcmsPluginInterface|string $plugin either the class name of a
     *     plugin to instantiate or a plugin instance
     *
     * @return WwppcmsPluginInterface instance of the loaded plugin
     *
    ...
    public function loadPlugin($plugin)
    {
        ...
        if (!($plugin instanceof WwppcmsPluginInterface)) {
            throw new RuntimeException(
                ...
                . "Manually loaded plugins must implement 'WwppcmsPluginInterface'"
            );
        }
     ...
     * means that plugin B does stuff which plugin A requires. However, Wwppcms
     * loads plugins in alphabetical order, so events might get fired on
     * plugin A before plugin B.
     *
     * Hence plugins need to be sorted. Wwppcms sorts plugins using a dependency
    ...
    protected function sortPlugins()
    {
        ...
        $visitPlugin = function ($plugin) use (
            ...
            if (!isset($visitedPlugins[$pluginName])) {
                ...
                if (!isset($nativePlugins[$pluginName])) {
                    $dependencies[] = 'WwppcmsDeprecated';
                }
        ...
        if (isset($this->plugins['WwppcmsDeprecated'])) {
            $visitPlugin($this->plugins['WwppcmsDeprecated']);
        }
     ...
     *
     * Plugins SHOULD implement {@see WwppcmsPluginInterface}, but you MUST NOT
     * rely on it. For more information see {@see WwppcmsPluginInterface}.
     *
     * @see Wwppcms::loadPlugins()
     * @see Wwppcms::getPlugins()
     *
    ...
    /**
     * Loads config.yml and any other *.yml from Wwppcms::$configDir
     *
     * After loading {@path "config/config.yml"}, Wwppcms proceeds with any other
    ...
    /**
     * Sets Wwppcms's config before calling Wwppcms::run()
     *
     * This method allows you to modify Wwppcms's config without creating a
     * {@path "config/config.yml"} or changing some of its variables before
     * Wwppcms starts processing.
     *
     * You can call this method between {@see Wwppcms::__construct()} and
     * {@see Wwppcms::run()} only. Options set with this method cannot be
     ...
     *
     * @throws LogicException thrown if Wwppcms already started processing
     */
    public function setConfig(array $config)
    {
        if ($this->locked) {
            throw new LogicException("You cannot modify Wwppcms's config after processing has started");
        }
    ...
    /**
     * Loads a theme's config file (wwppcms-theme.yml)
     *
     * @see Wwppcms::getTheme()
     * @see Wwppcms::getThemeApiVersion()
     */
    protected function loadTheme()
    {
        ...
        // load theme config from wwppcms-theme.yml
        $themeConfigFile = $this->getThemesDir() . $this->getTheme() . '/wwppcms-theme.yml';
        ...
        if (!isset($this->plugins['WwppcmsDeprecated']) && ($this->themeApiVersion < static::API_VERSION)) {
            throw new RuntimeException(
                'Current theme "' . $this->theme . '" uses API version ' . $this->themeApiVersion . ', but Wwppcms '
                . 'provides API version ' . static::API_VERSION . ' and WwppcmsDeprecated isn\'t loaded'
            );
        }
    }
     ...
     *
     * Wwppcms uses the `QUERY_STRING` routing method (e.g. `/wwppcms/?sub/page`)
     ...
     *
     * With Wwppcms 1.0 you had to setup URL rewriting (e.g. using `mod_rewrite`
     * on Apache) in a way that rewritten URLs follow the `QUERY_STRING`
     * principles. Starting with version 2.0, Wwppcms additionally supports the
     * `REQUEST_URI` routing method, what allows you to simply rewrite all
     * requests to just `index.php`. Wwppcms then reads the requested page from
     ...
     *
     * Wwppcms 0.9 and older required Apache with `mod_rewrite` enabled, thus old
     * plugins, templates and contents may require you to enable URL rewriting
     * to work. If you're upgrading from Wwppcms 0.9, you will probably have to
     ...
     *
     * Heads up! Wwppcms always interprets the first parameter as name of the
     * requested page (provided that the parameter has no value). According to
     * that you MUST NOT call Wwppcms with a parameter without value as first
     * parameter (e.g. http://example.com/wwppcms/?someBooleanParam), otherwise
     * Wwppcms interprets `someBooleanParam` as name of the requested page. Use
     * `/wwppcms/?someBooleanParam=` or `/wwppcms/?index&someBooleanParam` instead.
     *
     * @see Wwppcms::getRequestUrl()
     */
    protected function evaluateRequestUrl()
    {
        // use QUERY_STRING; e.g. /wwppcms/?sub/page
        ...
        // use REQUEST_URI (requires URL rewriting); e.g. /wwppcms/sub/page
     ...
     *
     * @see Wwppcms::evaluateRequestUrl()
     *
    ...
    public function resolveFilePath($requestUrl)
    {
        ...
        if (!$requestUrl) {
            ...
            if (!$requestFile) {
                ...
                // file as being requested (maintains backward compatibility to Wwppcms < 1.0)
    ...
    public function load404Content($file)
    {
        ...
        if (substr($file, 0, $contentDirLength) === $contentDir) {
            ...
            // content directory, fallback to Wwppcms's global `404.md`
    ...
    /**
     * Returns TRUE when Wwppcms is serving a 404 page
     *
     * @see Wwppcms::load404Content()
     *
     * @return bool TRUE if Wwppcms is serving a 404 page, FALSE otherwise
     */
     ...
     * by calling the {@see Wwppcms::substituteFileContent()} method.
     *
    ...
    /**
     * Reads the data of all pages known to Wwppcms
     *
     ...
     * ({@see Wwppcms::discoverPageSiblings()}). The `tree_node` key doesn't
     * exit until the `onPageTreeBuilt` event is triggered
     * ({@see Wwppcms::buildPageTree()}).
     *
     * @see Wwppcms::sortPages()
     * @see Wwppcms::discoverPageSiblings()
     * @see Wwppcms::getPages()
     */
    ...
    /**
     * Sorts all pages known to Wwppcms
     *
     * @see Wwppcms::readPages()
     * @see Wwppcms::getPages()
     */
     ...
     *
     * @see Wwppcms::discoverCurrentPage()
     *
     ...
     *
     * Wwppcms's page tree is a list of all the tree's branches (no matter the
     ...
     * Wwppcms's main index page (i.e. `index.md`) is represented by the tree's
     ...
     * The order of a node's children matches the order in Wwppcms's pages array.
     *
     ...
     * non-iterable data structure with Wwppcms 3.0.
     *
     * @see Wwppcms::getPageTree()
     */
     ...
     * registers Wwppcms's core Twig filter `content` as well as Wwppcms's
     * {@see WwppcmsTwigExtension} Twig extension.
     *
     * @see Wwppcms::getTwig()
    ...
    public function getTwig()
    {
        if ($this->twig === null) {
            ...
            $this->twig->addExtension(new WwppcmsTwigExtension($this));
            ...
            // this is the reason why we can't register this filter as part of WwppcmsTwigExtension
            $wwppcms = $this;
            $pages = &$this->pages;
            $this->twig->addFilter(new Twig_SimpleFilter(
                'content',
                function ($page) use ($wwppcms, &$pages) {
                    if (isset($pages[$page])) {
                        $pageData = &$pages[$page];
                        if (!isset($pageData['content'])) {
                            $markdown = $wwppcms->prepareFileContent($pageData['raw_content'], $pageData['meta']);
                            $pageData['content'] = $wwppcms->parseFileContent($markdown);
                        }
    ...
    /**
     * Returns the base URL of this Wwppcms instance
     *
     * Security Notice: You MUST configure Wwppcms's base URL explicitly when
    ...
    /**
     * Returns TRUE if Wwppcms's debug mode is enabled
     *
     * @return bool TRUE if Wwppcms's debug mode is enabled, FALSE otherwise
     */
    ...
    /**
     * Returns the URL of the themes folder of this Wwppcms instance
     *
     * @see Wwppcms::getUrlFromPath()
     *
     * @deprecated 2.1.0
     *
     * @return string
    ...
    /**
     * Returns the URL of a given absolute path within this Wwppcms instance
     *
     ...
     * a sub folder of {@see Wwppcms::$rootDir} (what is often identical to the
     ...
     * us to use {@see Wwppcms::getBaseUrl()} as origin for the URL.
     *
     * This method is used to guess Wwppcms's `plugins_url`, `themes_url` and
     ...
     *
     * This method is just an alias for {@see Wwppcms::filterVariable()}, see
     * {@see Wwppcms::filterVariable()} for a detailed description. It can be
     * used in Twig templates by calling the `url_param` function.
     *
     * @see Wwppcms::filterVariable()
     *
     ...
     *     sorted; use Wwppcms::SORT_ASC for a alphabetical ascending order (this
     *     is the default behaviour), Wwppcms::SORT_DESC for a descending order
     *     or Wwppcms::SORT_NONE to leave the result unsorted
     *
     ...
     *     use Wwppcms::SORT_ASC for a alphabetical ascending order (this is the
     *     default behaviour), Wwppcms::SORT_DESC for a descending order or
     *     Wwppcms::SORT_NONE to leave the result unsorted
     *
     * @return array list of found files
     */
    ...
    /**
     * Makes a relative path absolute to Wwppcms's root dir
     *
     * @param string $path     relative or absolute path
     * @param string $basePath treat relative paths relative to the given path;
     *     defaults to Wwppcms::$rootDir
    ...
    /**
     * Makes a relative URL absolute to Wwppcms's base URL
     *
     ...
     *     defaults to Wwppcms::getBaseUrl()
     * @param bool $endSlash   whether to add a trailing slash to the absolute
     ...
     *
     * Plugins using older API versions are handled by {@see WwppcmsDeprecated}.
     * Please note that {@see WwppcmsDeprecated} also triggers custom events on
     ...
     *
     * You MUST NOT trigger events of Wwppcms's core with a plugin!
     *
     * @see WwppcmsPluginInterface
     * @see AbstractWwppcmsPlugin
     * @see DummyPlugin
     *
