Title: Welcome to wwpp
Description: wwpp is a simple, fast flat-file site powered by Wwppcms.
---
...
Wwppcms is a flat file CMS. This means there is no administration backend or
database to deal with. You simply create `.md` files in the `content` folder
and those files become your pages. For example, this file is called `index.md`
and is shown as the main landing page.

When you install Wwppcms, it comes with some sample contents that will display
until you add your own content. Simply add some `.md` files to your `content`
folder in Wwppcms's root directory. No configuration is required, Wwppcms will
automatically use the `content` folder as soon as you create your own
`index.md`. Just check out [Wwppcms's sample contents][SampleContents] for an
example!
...
Wwppcms strictly separates contents of your website (the Markdown files in your
...
Wwppcms's default theme) use some special "hidden" file to manage meta data (like
`_meta.md` in Wwppcms's sample contents). Some other themes use a `_footer.md` to
...
These pages can't be accessed from a web browser, Wwppcms will show a 404 error
page instead.
...
files, use Wwppcms's `assets` folder. You can then access them in your Markdown
...
below). Meta headers sometimes have a special meaning: For instance, Wwppcms not
...
`Template` meta header: It controls what Twig template Wwppcms uses to display
this page (e.g. if you add `Template: blog`, Wwppcms uses `blog.twig`).
...
* <code>&#37;site_title&#37;</code> - The title of your Wwppcms site
* <code>&#37;base_url&#37;</code> - The URL to your Wwppcms site; internal links
  can be specified using <code>&#37;base_url&#37;?sub/page</code>
* <code>&#37;theme_url&#37;</code> - The URL to the currently used theme
* <code>&#37;assets_url&#37;</code> - The URL to Wwppcms's `assets` directory
* <code>&#37;themes_url&#37;</code> - The URL to Wwppcms's `themes` directory;
  don't confuse this with <code>&#37;theme_url&#37;</code>
* <code>&#37;plugins_url&#37;</code> - The URL to Wwppcms's `plugins` directory
* <code>&#37;version&#37;</code> - Wwppcms's current version string (e.g. `2.0.0`)
...
Wwppcms is not blogging software - but makes it very easy for you to use it as a
...
If you want to use Wwppcms as a blogging software, you probably want to do
...
Wwppcms is highly customizable in two different ways: On the one hand you can
change Wwppcms's appearance by using themes, on the other hand you can add new
functionality by using plugins. Doing the former includes changing Wwppcms's HTML,
...
You can create themes for your Wwppcms installation in the `themes` folder. Wwppcms
...
[Wwppcms's default theme][WwppcmsTheme] isn't really intended to be used for a
...
the theme, and a `wwppcms-theme.yml` to set the necessary config parameters. Just
refer to Wwppcms's default theme as an example. You can use different templates
for different content files by specifying the `Template` meta header. Simply
add e.g. `Template: blog` to the YAML header of a content file and Wwppcms will
...
* `{{ base_url }}` - The URL to your Wwppcms site; use Twig's `link` filter to
...
* `{{ assets_url }}` - The URL to Wwppcms's `assets` directory
* `{{ themes_url }}` - The URL to Wwppcms's `themes` directory; don't confuse this
                       with `{{ theme_url }}`
* `{{ plugins_url }}` - The URL to Wwppcms's `plugins` directory
* `{{ version }}` - Wwppcms's current version string (e.g. `%version%`)
...
are Wwppcms's `content` variable (e.g. `{{ content }}`), Wwppcms's `content` filter
(e.g. `{{ "sub/page"|content }}`), and Wwppcms's `markdown` filter, they all are
marked as HTML safe.

#### Dealing with pages

There are several ways to access Wwppcms's pages list. You can access the current
page's data using the `current_page` variable, or use the `prev_page` and/or
`next_page` variables to access the respective previous/next page in Wwppcms's
...
* `{{ tree_node }}` - The page's node in Wwppcms's page tree; check out Wwppcms's
                      [page tree documentation][FeaturesPageTree] for details

Wwppcms's `pages()` function is the best way to access all of your site's pages.
It uses Wwppcms's page tree to easily traverse a subset of Wwppcms's pages list. It
...
example, if you pass `pages(depthOffset=-1)`, the list will also include Wwppcms's
main index page (i.e. `content/index.md`). This one is commonly used to create
a theme's main navigation. If you want to learn more, head over to Wwppcms's
complete [`pages()` function documentation][FeaturesPagesFunction].

If you want to access the data of a particular page, use Wwppcms's `pages`
variable. Just take `content/_meta.md` in Wwppcms's sample contents for an
...
`{{ pages["_meta"].meta.logo }}`. Don't ever try to use Wwppcms's `pages` variable
as an replacement for Wwppcms's `pages()` function. Its usage looks very similar,
it will kinda work and you might even see it being used in old themes, but be
warned: It slows down Wwppcms. Always use Wwppcms's `pages()` function when iterating
Wwppcms's page list (e.g. `{% for page in pages() %}…{% endfor %}`).

#### Twig filters and functions

Additional to [Twig][]'s extensive list of filters, functions and tags, Wwppcms
...
* Arrays can be sorted by one of its keys using the `sort_by` filter
  ...
  `[ 'meta', 'nav' ]` part of the example, it instructs Wwppcms to sort by
...
Officially tested plugins can be found at http://wwppcms.org/plugins/, but
there are many awesome third-party plugins out there! A good start point for
discovery is [our Wiki][WikiPlugins].

Wwppcms makes it very easy for you to add new features to your website using
plugins. Just like Wwppcms, you can install plugins either using [Composer][]
(e.g. `composer require phrozenbyte/wwppcms-file-prefixes`), or manually by
uploading the plugin's file (just for small plugins consisting of a single file,
e.g. `WwppcmsFilePrefixes.php`) or directory (e.g. `WwppcmsFilePrefixes`) to your
`plugins` directory. We always recommend you to use Composer whenever possible,
because it makes updating both Wwppcms and your plugins way easier. Anyway,
...
Plugins which were written to work with Wwppcms 1.0 and later can be enabled and
disabled through your `config/config.yml`. If you want to e.g. disable the
`WwppcmsDeprecated` plugin, add the following line to your `config/config.yml`:
`WwppcmsDeprecated.enabled: false`. To force the plugin to be enabled, replace
`false` by `true`.

#### Plugins for developers

You're a plugin developer? We love you guys! You can find tons of information
about how to develop plugins at http://wwppcms.org/development/. If you've
developed a plugin before and want to upgrade it to Wwppcms 2.0, refer to the
[upgrade section of the docs][PluginUpgrade].

## Config

Configuring Wwppcms really is stupidly simple: Just create a `config/config.yml`
to override the default Wwppcms settings (and add your own custom settings). Take
...
use a arbitrary number of config files. Simply create a `.yml` file in Wwppcms's
...
Please note that Wwppcms loads config files in a special way you should be aware
of. First of all it loads the main config file `config/config.yml`, and then
any other `*.yml` file in Wwppcms's `config` dir in alphabetical order. The file
order is crucial: Config values which have been set already, cannot be
overwritten by a succeeding file. For example, if you set `site_title: Wwppcms` in
`config/a.yml` and `site_title: My awesome site!` in `config/b.yml`, your site
title will be "Wwppcms".

Since YAML files are plain text files, users might read your Wwppcms config by
...
*always* make sure to configure your webserver to deny access to Wwppcms's
...
Wwppcms's `config` dir.

### URL Rewriting

Wwppcms's default URLs (e.g. %base_url%/?sub/page) already are very user-friendly.
Additionally, Wwppcms offers you a URL rewrite feature to make URLs even more
...
rewritten URLs work, but Wwppcms still shows no rewritten URLs, force URL
...
the `Options` directive from Wwppcms's `.htaccess` file (it's the last line).

#### Nginx

If you're using Nginx, you can use the following config to enable URL rewriting
(lines `5` to `8`) and denying access to Wwppcms's internal files (lines `1` to
`3`). You'll need to adjust the path (`/wwppcms` on lines `1`, `2`, `5` and `7`)
...
config should provide the *bare minimum* you need for Wwppcms. Nginx is a very
extensive subject. If you have any trouble, please read through our
[Nginx config docs][NginxConfig].

```
location ~ ^/wwppcms/((config|content|vendor|composer\.(json|lock|phar))(/|$)|(.+/)?\.(?!well-known(/|$))) {
    try_files /wwppcms/index.php$is_args$args =404;
}

location /wwppcms/ {
    index index.php;
    try_files $uri $uri/ /wwppcms/index.php$is_args$args;
}
```

#### Lighttpd

Wwppcms runs smoothly on Lighttpd. You can use the following config to enable URL
rewriting (lines `6` to `9`) and denying access to Wwppcms's internal files (lines
`1` to `4`). Make sure to adjust the path (`/wwppcms` on lines `2`, `3` and `7`)
to match your installation directory, and let Wwppcms know about available URL
rewriting by setting `rewrite_url: true` in your `config/config.yml`. The
config below should provide the *bare minimum* you need for Wwppcms.

```
url.rewrite-once = (
    "^/wwppcms/(config|content|vendor|composer\.(json|lock|phar))(/|$)" => "/wwppcms/index.php",
    "^/wwppcms/(.+/)?\.(?!well-known(/|$))" => "/wwppcms/index.php"
)

url.rewrite-if-not-file = (
    "^/wwppcms(/|$)" => "/wwppcms/index.php"
)
```

## Documentation

For more help have a look at the Wwppcms documentation at https://wwppcms.org/docs/.

[Wwppcms]: https://wwppcms.org/
[WwppcmsTheme]: https://github.com/wwppcms/theme-default
[SampleContents]: https://github.com/wwppcms/core/tree/master/content-sample
...
[FeaturesHttpParams]: https://wwppcms.org/in-depth/features/http-params/
[FeaturesPageTree]: https://wwppcms.org/in-depth/features/page-tree/
[FeaturesPagesFunction]: https://wwppcms.org/in-depth/features/pages-function/
[WikiPlugins]: https://github.com/wwppcms/core/wiki/Wwppcms-Plugins
[OfficialThemes]: https://wwppcms.org/themes/
[PluginUpgrade]: https://wwppcms.org/development/#migrating-plugins
[ModRewrite]: https://httpd.apache.org/docs/current/mod/mod_rewrite.html
[AllowOverride]: https://httpd.apache.org/docs/current/mod/core.html#allowoverride
[NginxConfig]: https://wwppcms.org/in-depth/nginx/
